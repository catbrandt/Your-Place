...
describe('Events API', () => {
  ...
  test('Delete (204 / 403)', async () => {
    ...
    const forbidden = await request(app).delete(`/events/${otherEvent.id}`).set(authHeader());
    expect(forbidden.status).toBe(403);

    const ok = await request(app).delete(`/events/${ownedEvent.id}`).set(authHeader());
